...
def science_in_cooking():
    ...
    physics = '달고나속 물리 |n 달고나를 만들때 가스불의 열이 달고나 국자로 전도의 방식으로 열이 전달되어 달고나가 녹을수 있게 한다 . |n 전도: 물체 내부에서 에너지가 이동하는 현상'  
    
    chemistry = '달고나 속의 화학 |n 달고나 속의 화학은 달고나를 만들때 사용되는 소다(탄산 수소 나트륨)이 열분해를 통해 탄산 나트륨과 물분자, 이산화 탄소로 분해되었다 |n 화학식: 2NaHCO3(탄산 수소 나트륨) → NaCo3(탄산 나트륨) + H2O(물 분자) + CO2(이산화 탄소)'
    
    earth = '달고나 속의 지구 |n'
    living_things = '달고나 속의 생명 |n'
